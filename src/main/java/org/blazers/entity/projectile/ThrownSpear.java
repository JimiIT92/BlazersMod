...
/**
 * {@link BlazersMod Blazers Mod} {@link AbstractArrow Thrown Spear Entity}
 */
public final class ThrownSpear extends AbstractArrow {
     ...
     */
    public ThrownSpear(final EntityType<? extends AbstractArrow> type, final Level level) {
     ...
     * @param itemStack {@link ItemStack The Spear Item Stack}
     */
    public ThrownSpear(final EntityType<? extends AbstractArrow> type, final Level level, final LivingEntity entity, final ItemStack itemStack) {
        super(type, entity, level, itemStack);
    ...
    protected @NotNull ItemStack getDefaultPickupItem() {
        return BLItems.SPEAR.get().getDefaultInstance();
    }
     ...
     */
    @Nullable
    protected EntityHitResult findHitEntity(final @NotNull Vec3 startVec, final @NotNull Vec3 endVec) {
    ...
    protected void onHitEntity(final EntityHitResult hitResult) {
        ...
        if (entity instanceof LivingEntity livingEntity) {
            damage += EnchantmentHelper.getDamageBonus(this.spearItem, livingEntity.getType());
        }

        final Entity owner = this.getOwner();
        final DamageSource damageSource = new DamageSource(this.level().registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(BLDamageTypes.SPEAR), this, owner == null ? this : owner);
        this.dealtDamage = true;
     ...
     */
    protected boolean tryPickup(final @NotNull Player player) {
     ...
     *
     * @param nbt {@link CompoundTag Compound Tag}
     */
    public void readAdditionalSaveData(final @NotNull CompoundTag nbt) {
        super.readAdditionalSaveData(nbt);
        if (nbt.contains("Spear")) {
            this.spearItem = ItemStack.parseOptional(this.registryAccess(), nbt.getCompound("Spear"));
        }
    ...
    public void addAdditionalSaveData(final @NotNull CompoundTag nbt) {
        ...
        nbt.put("Spear", this.spearItem.save(this.registryAccess()));
